...
Connect a hub in **Settings → Usage Hubs** and each Claude account it pools gets its own card,
labeled with its email and the hub. Session, Weekly, model limits, and Extra Usage read through the
hub with that account's token, and the plan badge comes from Anthropic's profile; the spend tiles
and Usage Trend are not shown, since the session logs live on the hub. An account that is also
signed in on this Mac keeps its local card.
